...
kubectl apply -f k8s
kubectl set image deployments/client-deployment server=szczellak/multi-client:$SHA
kubectl set image deployments/server-deployment server=szczellak/multi-server:$SHA
kubectl set image deployments/worker-deployment server=szczellak/multi-worker:$SHA